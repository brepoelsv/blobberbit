...
const shownamebuttons = $('.button-showname');
const shownameicon = $('.button-showname > i');
...
for (let i = 0; i < shownamebuttons.length; i++) {
  shownamebuttons[i].onclick = () => {
    toggleName();
    buttons(shownamebuttons[0], 'uk-active', shownamebuttons, shownameicon);
    $.post('/settings', {
      option: 'displayname'
    });
  };
