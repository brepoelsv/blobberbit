...
function sendBonuses(node, writer) {
  if (node.bonuses.glue.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.glue.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.bomb.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.bomb.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.drunk.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.drunk.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.shoot.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.shoot.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.freeze.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.freeze.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.speed.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.speed.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.shield.active) {
    writer.writeUInt8(0x02);
    writer.writeUInt8(node.bonuses.shield.progress);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.extra.active) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.extra['1']) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.extra['2']) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.extra['3']) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.extra['4']) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.extra['5']) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.got.glue.active) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.got.freeze.active) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
  if (node.bonuses.got.drunk.active) {
    writer.writeUInt8(0x02);
  } else {
    writer.writeUInt8(0x01);
  }
...
UpdateNodes.prototype.writeUpdateItems11 = function (writer) {
  ...
  for (var i = 0; i < this.updNodes.length; i++) {
    ...
    if (node.cellType == 3) {
      flags |= 0x40;      // isEjected
    }
    if (node.cellType == 4) {
      flags |= 0x20;      // isBonus
    }
    writer.writeUInt16(flags >>> 0);         // Flags

    if (flags & 0x01) writer.writeUInt32(Math.round(node.owner._totalMass) >> 0);
  ...
  for (var i = 0; i < this.addNodes.length; i++) {
    ...
    if (flags & 0x20) writer.writeUInt8(node.bonusType >> 0);
