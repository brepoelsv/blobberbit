...
import objWithStats from '../lib/util';
...
import QuadNode  from './modules/QuadNode';
import Gamemode  from './gamemodes';
import PlayerTracker from './PlayerTracker';
import PacketHandler from './PacketHandler';
import PlayerCommand from './modules/PlayerCommand';
import Packet from './packet';

// GameServer implementation
function GameServer(io) {
  // Location of source files - For renaming or moving source files!
  this.srcFiles = __dirname;
  ...
  this.nodesBonusGlue = [];
  this.nodesBonusBomb = [];
  this.nodesBonusDrunk = [];
  this.nodesBonusShoot = [];
  this.nodesBonusX2 = [];
  this.nodesBonusX5 = [];
  this.nodesBonusExtra1 = [];
  this.nodesBonusExtra2 = [];
  this.nodesBonusExtra3 = [];
  this.nodesBonusExtra4 = [];
  this.nodesBonusExtra5 = [];
  this.nodesBonusFreeze = [];
  this.nodesBonusSpeed = [];
  this.nodesBonusShield = [];
  this.nodesBonusMega = [];
  ...
  this.config = {
    ...
    bonusGlueMinAmount: 0,
    bonusGlueMaxAmount: 0,
    bonusGlueValue: 0,
    bonusGlueTime: 0,
    bonusBombMinAmount: 0,
    bonusBombMaxAmount: 0,
    bonusDrunkMinAmount: 0,
    bonusDrunkMaxAmount: 0,
    bonusDrunkValue: 0,
    bonusDrunkTime: 0,
    bonusShootMinAmount: 0,
    bonusShootMaxAmount: 0,
    bonusShootValue: 0,
    bonusShootTime: 0,
    bonusX2MinAmount: 0,
    bonusX2MaxAmount: 0,
    bonusX5MinAmount: 0,
    bonusX5MaxAmount: 0,
    bonusExtra1MinAmount: 0,
    bonusExtra1MaxAmount: 0,
    bonusExtra2MinAmount: 0,
    bonusExtra2MaxAmount: 0,
    bonusExtra3MinAmount: 0,
    bonusExtra3MaxAmount: 0,
    bonusExtra4MinAmount: 0,
    bonusExtra4MaxAmount: 0,
    bonusExtra5MinAmount: 0,
    bonusExtra5MaxAmount: 0,
    bonusFreezeMinAmount: 0,
    bonusFreezeMaxAmount: 0,
    bonusFreezeTime: 0,
    bonusSpeedMinAmount: 0,
    bonusSpeedMaxAmount: 0,
    bonusSpeedValue: 0,
    bonusSpeedTime: 0,
    bonusShieldMinAmount: 0,
    bonusShieldMaxAmount: 0,
    bonusShieldTime: 0,
    bonusMegaMinAmount: 0,
    bonusMegaMaxAmount: 0
  };
...
GameServer.prototype.movePlayer = function(cell, client) {
  ...
  if (cell.bonuses.got.freeze.active)
    return;
  ...
  var time = this.config.playerRecombineTime,
      ...
      //base = Math.min(2, cell._size * 0.2) * 25;  // VB Faster recombine but more controlable than config Recombine
...
GameServer.prototype.spawnCells = function() {
  ...
  // spawn bonuses
  if (this.nodesBonusGlue.length < this.config.bonusGlueMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 1);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusBomb.length < this.config.bonusBombMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 2);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusDrunk.length < this.config.bonusDrunkMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 3);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusShoot.length < this.config.bonusShootMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 4);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusX2.length < this.config.bonusX2MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 5);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusX5.length < this.config.bonusX5MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 6);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusExtra1.length < this.config.bonusExtra1MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 7);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusExtra2.length < this.config.bonusExtra2MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 8);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusExtra3.length < this.config.bonusExtra3MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 9);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusExtra4.length < this.config.bonusExtra4MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 10);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusExtra5.length < this.config.bonusExtra5MinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 11);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusFreeze.length < this.config.bonusFreezeMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 12);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusSpeed.length < this.config.bonusSpeedMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 13);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusShield.length < this.config.bonusShieldMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 14);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
  if (this.nodesBonusMega.length < this.config.bonusMegaMinAmount) {
    const bonus = new Entity.Bonus(this, null, this.randomPos(), null, 15);
    if (!this.willCollide(bonus)) this.addNode(bonus);
  }
};
...
GameServer.prototype.startStatsServer = function() {
  // Create stats
  this.stats = "Test";
  this.getStats();
