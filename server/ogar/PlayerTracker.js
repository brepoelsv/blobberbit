...
function PlayerTracker(gameServer, socket) {
    ...
    this._totalMass = 0; // for skin changes when get bigger
    this._scale = 1;
...
PlayerTracker.prototype.getScale = function() {
    ...
    scale = Math.sqrt(mass * 100); // Has Effect on viewbox VB
    if (!scale) return scale = this._score = 0; // reset
    else {//console.log(Math.pow(Math.min(64 / scale, 1), 0.4));
...
PlayerTracker.prototype.updateTick = function() {
   ...
   //var scale = this.gameServer.config.serverMinScale;
...
PlayerTracker.prototype.sendUpdate = function() {
 ...
 //console.log("this.viewBox.minx + ", this.viewBox.minx, " this.viewBox.minx +", this.viewBox.minx, " this.viewBox.maxx + ", this.viewBox.maxx, " this.viewBox.maxy +", this.viewBox.maxy);
    ...
    for (; newIndex < this.viewNodes.length && oldIndex < this.clientNodes.length;) {
        ...
        if (node.isMoving || node.cellType == 0 || node.cellType == 2 || node.cellType == 5)
            updNodes.push(node);
        newIndex++;
        oldIndex++;
    }
    ...
    if (++this.tickMiniMap > 25) {
      // 1 / 0.040 = 25 (once per second)
      this.tickMiniMap = 0;
      packetHandler.sendPacket(new Packet.MiniMap(this, this.gameServer.clients));
    }
};
    ...
    if (!this.spectate || len) {
        ...
        if (this.freeRoam || this.getSpecTarget() == null) {
            ...
            if (player) {
              ...
              //console.log(this.viewBox.minx, " ", this.viewBox.miny, " ", this.viewBox.maxx, " ", this.viewBox.maxy);
            }
        }
        // sends camera packet
